...
pub const Options = struct {
    ...
    pub const SUB = struct {
        pub const SUBSCRIBE = c.NNG_OPT_SUB_SUBSCRIBE;
        pub const UNSUBSCRIBE = c.NNG_OPT_SUB_UNSUBSCRIBE;
        pub const PREFNEW = c.NNG_OPT_SUB_PREFNEW;
    };

    pub const REQ = struct {
        pub const RESENDTIME = c.NNG_OPT_REQ_RESENDTIME;
        pub const RESENDTICK = c.NNG_OPT_REQ_RESENDTICK;
    };
...
pub const Socket = struct {
    ...
    pub const REQ = Protocol(c.nng_req0_open);
    pub const REP = Protocol(c.nng_rep0_open);
};
...
fn Protocol(comptime constructor: *const fn (ptr: *c.nng_socket) callconv(.C) c_int) type {
    return struct {
        ...
        pub fn setBool(self: Self, opt: [*:0]const u8, val: bool) !void {
            try throw(c.nng_socket_set_bool(self.impl, opt, val));
        }

        pub fn setInt(self: Self, opt: [*:0]const u8, val: i32) !void {
            try throw(c.nng_socket_set_int(self.impl, opt, val));
        }

        pub fn setPtr(self: Self, opt: [*:0]const u8, val: ?*anyopaque) !void {
            try throw(c.nng_socket_set_ms(self.impl, opt, val));
        }

        pub fn setSize(self: Self, opt: [*:0]const u8, val: c.size_t) !void {
            try throw(c.nng_socket_set_size(self.impl, opt, val));
        }

        pub fn setString(self: Self, opt: [*:0]const u8, val: [*:0]const u8) !void {
            try throw(c.nng_socket_set_string(self.impl, opt, val));
        }
...
pub const Message = struct {
    ...
    pub fn dupe(self: Message) !Message {
        var ptr: ?*c.nng_msg = null;
        try throw(c.nng_msg_dup(&ptr, self.ptr));
        return .{ .ptr = ptr };
    }
};
